...
const MobileMenu = ({ menuOpen, setMenuOpen, user, handleLogout }) => {
  ...
  return (
    <motion.div
      ...
      className="md:hidden fixed inset-0 top-20 bg-primary z-40 shadow-2xl p-6 space-y-4 overflow-y-auto"
    >
      {/* Profile Section */}
      ...
      {allLinks.map((link) => (
        <NavLink
          key={link.to}
          to={link.to}
          className="block text-lg py-3 px-4 hover:bg-primary rounded-lg transition"
      ...
      <div className="border-t border-[#008080] my-2"></div>
      ...
      <button
        ...
        className="w-full text-lg py-3 px-4 text-red-400 hover:bg-primary rounded-lg transition flex items-center"
      >
